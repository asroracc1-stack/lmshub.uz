...
export default function DailyStreakCard({ data }: DailyStreakCardProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
      <Card className="p-5 flex flex-col justify-between h-full rounded-3xl shadow-sm border-slate-100">
        <div>
          <h3 className="text-sm font-semibold text-slate-800 flex items-center gap-1.5">
            Kunlik streak <Flame className="h-4 w-4 text-orange-500 fill-orange-500" />
          </h3>
          <p className="text-3xl font-bold text-slate-900 mt-2">{data.dailyStreak}</p>
          <p className="text-xs text-slate-500 mb-4">kun davom etmoqda</p>
        ...
        <div className="mt-5 bg-purple-50 rounded-2xl p-3 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <p className="text-[9px] text-slate-500">(15.04.2024)</p>
          </div>
        </div>
      </Card>
    </motion.div>
