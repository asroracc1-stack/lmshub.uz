...
export default function DashboardBanner({ data }: DashboardBannerProps) {
  return (
    <motion.div 
      ...
      className="bg-gradient-to-r from-teal-400 to-cyan-500 rounded-3xl p-6 text-white flex flex-col md:flex-row items-center justify-between shadow-lg relative overflow-hidden"
    >
      ...
      <div className="relative z-10 w-full md:w-2/3 space-y-6">
        ...
        <div className="flex items-center gap-8 md:gap-16">
          <div>
            <p className="text-teal-100 text-sm mb-1">IELTS Target</p>
            <p className="text-3xl font-bold">{data.targetBand}</p>
          </div>
          <div>
            <p className="text-teal-100 text-sm mb-1">Current Band</p>
            <p className="text-3xl font-bold">{data.currentBand || "—"}</p>
          </div>
          <div className="flex-1 max-w-[200px]">
            <p className="text-teal-100 text-sm mb-1">Progress</p>
        ...
        <div className="flex gap-3 pt-2">
          <Button variant="secondary" className="bg-white text-teal-600 hover:bg-white/90 gap-2 rounded-full font-semibold px-6">
