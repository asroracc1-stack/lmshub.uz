...
export default function AIAttendanceDashboard() {
  ...
  const fetchCameras = async () => {
    try {
      ...
      // Fallback fallback states for offline dev testing
      setCameras([
        { id: "1", name: "Auditorium Main A", ipAddress: "192.168.1.150", status: "ONLINE", pingLatencyMs: 12, packetLossPct: 0.0, protocol: "RTSP" },
        { id: "2", name: "Front Entry Corridor", ipAddress: "192.168.1.151", status: "OFFLINE", pingLatencyMs: undefined, packetLossPct: 100.0, protocol: "ONVIF" }
      ]);
  ...
  const fetchUnknowns = async () => {
    try {
      const res = await api.get("/unknowns"); // Mock or real endpoint
  ...
  const runCameraDiscovery = async () => {
    ...
    setTimeout(() => {
      ...
      toast.success("Discovery complete! 0 new cameras detected (ip leases checked).");
    }, 2500);
  };
  ...
  const saveConfig = () => {
    toast.success("AI Configuration Panel values saved successfully.");
  };

  const saveRules = () => {
    toast.success("Tenant Attendance Rule Engine rules applied globally.");
  };
  ...
  return (
    <div className="space-y-6 p-6 min-h-screen bg-slate-950 text-slate-100 font-sans">
      ...
      <div className="bg-slate-900/60 backdrop-blur-md p-6 rounded-2xl border border-slate-800 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent flex items-center gap-2">
            ...
            AI Attendance Administration Panel
          </h1>
          <p className="text-slate-400 text-sm mt-1">Enterprise management dashboard for cameras, liveness settings, and neural models.</p>
        </div>
      ...
      <div className="flex border-b border-slate-800 gap-1">
        {[
          { id: "cameras", label: "Camera Monitor", icon: Camera },
          { id: "config", label: "AI Config", icon: Sliders },
          { id: "rules", label: "Attendance Rules", icon: Settings },
          { id: "unknowns", label: "Unknown Persons", icon: AlertTriangle }
        ].map((tab: any) => {
          const Icon = tab.icon;
          return (
            <button
      ...
      <div className="mt-6">
              ...
              <Card key={cam.id} className="p-6 bg-slate-900/40 border border-slate-800 hover:border-slate-700 transition duration-300 rounded-xl relative overflow-hidden group">
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-slate-500 tracking-wider uppercase">{cam.protocol} Stream</span>
                ...
                <div className="grid grid-cols-2 gap-4 mt-6 pt-4 border-t border-slate-800 text-sm">
                  <div>
                    <p className="text-slate-500 font-medium">Ping Latency</p>
                    <p className="text-slate-200 font-semibold font-mono mt-0.5">
                      {cam.pingLatencyMs ? `${cam.pingLatencyMs} ms` : "—"}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-500 font-medium">Packet Loss</p>
                    <p className="text-slate-200 font-semibold font-mono mt-0.5">
        ...
        {activeTab === "config" && (
          <Card className="p-8 bg-slate-900/40 border border-slate-800 rounded-xl space-y-6 max-w-2xl">
            <h3 className="text-xl font-bold text-slate-100 flex items-center gap-2">
              <Sliders className="h-5 w-5 text-indigo-400" /> Neural Network Sensitivity Config
            </h3>
            
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-400">Cosine Similarity Confidence Match: {confidence.toFixed(2)}</label>
              ...
              <p className="text-xs text-slate-500">Above this score, matching vector is auto-marked as present.</p>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-400">Snapshot Processing Interval: {interval} seconds</label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-400">Liveness Sensitivity Level</label>
              <select value={liveness} onChange={(e) => setLiveness(e.target.value)} className="w-full bg-slate-900 border border-slate-800 rounded-lg px-4 py-2 text-slate-100 focus:outline-none focus:border-indigo-500">
                ...
                <option value="MEDIUM">MEDIUM (Texture + moiré check)</option>
                <option value="HIGH">HIGH (Texture + moiré + specularity check)</option>
              </select>
            </div>

            <Button className="bg-indigo-600 hover:bg-indigo-500 w-full" onClick={saveConfig}>Save Sensitivity Rules</Button>
          </Card>
        )}
        ...
        {activeTab === "rules" && (
          <Card className="p-8 bg-slate-900/40 border border-slate-800 rounded-xl space-y-6 max-w-2xl">
            <h3 className="text-xl font-bold text-slate-100 flex items-center gap-2">
              <Activity className="h-5 w-5 text-indigo-400" /> Attendance Rule Engine Settings
            </h3>
            
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-400">Late Arrival Minutes Threshold</label>
              ...
              <p className="text-xs text-slate-500">Check-in time after this limit will mark the student as LATE.</p>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-400">Absence Minutes Threshold</label>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-slate-400">Minimum Presence score: {(minScore * 100).toFixed(0)}%</label>
            ...
            <Button className="bg-indigo-600 hover:bg-indigo-500 w-full" onClick={saveRules}>Apply Attendance Limits</Button>
          </Card>
        )}
            ...
            {unknowns.map(un => (
              <Card key={un.id} className="p-4 bg-slate-900/40 border border-slate-800 hover:border-slate-700 transition duration-200 rounded-xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-200">Incident Detected: Unknown Person</h4>
                    <p className="text-xs text-slate-500 font-mono mt-0.5">Matched Score: {(un.closestMatchScore * 100).toFixed(0)}% | Time: {new Date(un.detectedAt).toLocaleTimeString()}</p>
                  </div>
                </div>

                <div className="flex gap-2">
                  <Button size="sm" className="bg-emerald-600 hover:bg-emerald-500 text-xs">Verify Student</Button>
                  <Button size="sm" variant="outline" className="border-rose-950/40 text-rose-400 hover:bg-rose-500/10 text-xs">Security Alert</Button>
                </div>
              </Card>
            ))}
          </div>
