...
import { useAdminDashboard, useAdminDashboardStats } from "@/hooks/useOptimizedQueries";
...
export default function AdministratorDashboard() {
  ...
  const { data: realStats, isLoading: statsLoading } = useAdminDashboardStats();
          ...
          {Array.from({ length: 6 }).map((_, i) => (
            <Card key={i} className="flex flex-col justify-between p-5 min-h-[145px] bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 animate-pulse relative overflow-hidden">
              <div className="flex items-start justify-between mb-3">
                <Skeleton className="h-10 w-10 rounded-lg bg-slate-200 dark:bg-slate-700 animate-pulse" />
              </div>
              <div>
