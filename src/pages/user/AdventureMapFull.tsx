...
import { ArrowLeft, RefreshCw, Compass, Trophy, Star, Shield, Gift, Flame, MapPin, ChevronRight } from "lucide-react";
      ...
      <div className={`flex items-center justify-between gap-4 border p-4 rounded-3xl backdrop-blur-md transition-all duration-300 ${
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-bold">{t("learningWorld.subtitle")}</p>
          </div>
        </div>
              ...
              {nextReward && (
                <div className={`p-3 rounded-2xl border ${
                  ...
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 block mb-1">
                    {t("learningWorld.nextReward")}
                  </span>
                  <div className="flex items-center gap-2">
                    <Gift className="w-4 h-4 text-amber-500 animate-bounce" />
                    <span className="text-xs font-bold">
                      {nextReward.reward_type === "COIN_BOX" ? `${nextReward.reward_value} ${t("learningWorld.coins")}` : `${t("learningWorld.rewardChest")}`}
                    </span>
                  </div>
                </div>
              )}
